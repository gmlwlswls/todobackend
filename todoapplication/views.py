...
#JsonResponse로 JSON 데이터를 출력하고자 하면 
#빠르게 JSON 문자열로 만들때 dict만 가능하기 때문
#dict는 JSON문자열과 표현 방법이 같기 때문입니다.
...
class TodoView(View) :
  ...
  def put(self,request) :
    ...
    #user id 와 id 그리고 done 값을 읽어서 저장
    #클라이언트에서 입력해주는 데이터만 읽어오면 됩니다.
    userid = request["userid"]
    id = request["id"]
    done = request["done"]

    #모델 인스턴스 생성(POST일 때는 todo = ToDo())
    #수정할 데이터를 찾아옵니다.
